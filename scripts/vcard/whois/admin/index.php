...
class EP_Dev_Whois_Admin
{
	...
	function page_FAQ()
	{
		...
		. highlight_string("<? header(\"Content-Type: text/html; charset=UTF-8\"); ?\>", true) . "</span>";
			...
			if ($extension_count < $requests_per_page) // ($start_number+$requests_per_page)
			{
				...
				foreach($whois_query->getAllServerNames() as $current_server)
				{
					if (($server != "all" && $server == $current_server) || ($server == "all"))
					{
						...
						<div style=\"margin: 15px;\">" . nl2br(htmlentities($whois_query->getWhoisReport(false))) . "</div></div>\r\n</td>\r\n</tr>\r\n";
...
JAVASCRIPT_CODE;
	...
	function getDonationBox()
	{
		$content .= "<div style=\"border: 3px solid #268CFE; background-color: #F0F3FE; margin: 10px; padding: 5px;\">
			...
			<div>Help support this script by making a donation. While this script is released free of charge, many hours of development have went into the making of this script over the past few years. These development hours are unpaid and therefore are not beneficial in obtaining, for example, food or shelter to live on. If you cannot donate now, please still feel free to use the script. You can find the link for donation in the lower left-hand menu of this page.</div><br>
