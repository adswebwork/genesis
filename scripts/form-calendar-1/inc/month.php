<?php
	...
	$leading_zero	= $_POST['leadingZero'] ? true : false;
	$two_digit_year	= $_POST['twoDigitYear'] ? true : false;
	
	$ts 			= $_POST['ts'] ? $_POST['ts'] : mktime(1, 1, 1, date('n'), 1, date('Y'));
	
	$t_ts 			= mktime(1, 1, 1, date('n'), date('j'), date('Y'));	//Timestamp today
		
	$ts_year_full	= date('Y', $ts);
	$ts_year		= date($two_digit_year ? 'y' : 'Y', $ts);
	$ts_month_nr	= date($leading_zero ? 'm' : 'n', $ts);
...
<table class="month<?=($picker ? ' picker' : '')?>" cellpadding="0" summary="{'ts': '<?=$ts?>', 'pr_ts': '<?=$pr_ts?>', 'nx_ts': '<?=$nx_ts;?>', 'label': '<?=$ts_month.', '.$ts_year_full?>', 'current': 'month', 'parent': 'year'<?=($ts_year == 1979 && date('n', $ts) == 1 ? ", 'hide_left_arrow': '1'" : '').($ts_year == 2030 && date('n', $ts) == 12 ? ", 'hide_right_arrow': '1'" : '')?>}">
	...
	for($i = 0; $i < $wdays_counter; $i++) {
		...
		echo '<td class="outsideDay" date="'."{'day': '".$day."', 'month': '".date($leading_zero ? 'm' : 'n', $i_ts)."', 'year': '".date($two_digit_year ? 'y' : 'Y', $i_ts)."'}".'">'.$day.'</td>';
	}
	...
	for($i = 1; $i <= $ts_nrodays; $i++) {
		...
		echo '<td'.($i_ts == $t_ts ? ' class="today"' : '').' '."date=\"{'day': '".($leading_zero && $i < 10 ? '0'.$i : $i)."', 'month': '".$ts_month_nr."', 'year': '".$ts_year."'}\">".$i.'</td>';
	...
	if($wdays_counter != 0) {
		for($i = $wdays_counter; $i < 7; $i++) {
			...
			echo '<td class="outsideDay" date="'."{'day': '".($leading_zero && $a < 10 ? '0'.$a : $a)."', 'month': '".date($leading_zero ? 'm' : 'n', $i_ts)."', 'year': '".date($two_digit_year ? 'y' : 'Y', $i_ts)."'}".'">'.$a.'</td>';
			$a++;
		}
		$row++;
	}
