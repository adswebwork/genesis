...
function RenameFileIfNeccesary($oldfilename) {
	...
	$output  = '<font color="blue">Parsing <b>'.htmlentities($oldfilename, ENT_QUOTES).'</b></font><br>';
	...
	if (eregi('^phpThumb_cache\.(.*)\.([0-9]+)\.([0-9]+)\.([0-9]+)\.([0-9]+)\.([0-9]+)\.([0-9]+)\.([0-9]+)\.([0-9]+)\.(jpeg|png|gif)$', $oldbasefilename, $matches)) {
		...
		$output .= '<font color="green">matched filename structure for v1.4.1+ (no need to rename)</font><br>';
	...
	if (!empty($FileData)) {
		...
		$cache_filename .= '_'.$FileData['quality'];
...
if (!empty($_POST['cachedir'])) {
	if (is_dir($_POST['cachedir'])) {
		if ($dir = opendir($_POST['cachedir'])) {
			while ($fileName = readdir($dir)) {
				if (ereg('^phpThumb_cache\.', $fileName)) {
					RenameFileIfNeccesary(realpath($_POST['cachedir']).'/'.$fileName);
				}
			}
		}
	}
