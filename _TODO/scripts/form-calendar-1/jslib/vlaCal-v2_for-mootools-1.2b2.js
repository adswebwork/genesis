var vlaCalendar = new Class({
			...
			case 'month':
				if(this.picker) {
					clickables.each(function(_clickable) {
						_clickable.onclick = function() { _class.pick(JSON.decode(_clickable.getProperty('date'))); }
					});
				}
				break;
			case 'year':
	...
	u: function(_url, _pars, _onComplete, _id) {
		if(!this.loading) {
			...
			_pars += '&picker=' + (this.picker ? 1 : 0) + '&startMonday=' + (this.startMonday ? 1 : 0) + '&leadingZero=' + (this.leadingZero ? 1 : 0)  + '&twoDigitYear=' + 
					 (this.twoDigitYear ? 1 : 0) + '&style=' +  this.style;
...
var vlaDatePicker = vlaCalendar.extend({
	...
	position: function() {
		...
		switch(this.alignY) {
			case 'bottom':
				top = this.element.getTop() + this.element.getSize().y;
				break;
			case 'top': 
	...
	show: function() {
		...
		if(!this.visible) {
			...
			if(Browser.Engine.trident5) this.picker.setStyle('background-color', this.ieTransitionColor); // <- Ugly transition fix for IE browsers
			this.picker.effect('opacity', {duration: this.fadeDuration, transition: this.transition}).start(0, 1)
				.addEvent('onComplete', function() { if(Browser.Engine.trident5) _class.picker.setStyle('background-color', 'transparent'); _class.outsideHide = true; });
		}
	},
